...
class MyStreamListener(StreamListener):
  ...
  def on_error(self, status_code):
    ...
    try:
        if status_code == 420:
            ...
            #Writing 420 error code information into the file at '/dbfs/FileStore/tables/Streaming_Process/Dev/Error_Log_Files/'
            Filename = '/dbfs/FileStore/tables/Streaming_Process/Dev/Error_Log_Files/Error_on_'
            Date = str(date.today())
            ...
            try:
                RDS_HOSTNAME = secret["Rds_Hostname_Dev"]
                DBUSERNAME = secret["Db_Username_Dev"]
                DBPASSWORD = secret["Db_Password_Dev"]
                DATABASE = secret["Database_Dev"]
                CONNECTION_TIMEOUT = secret["Db_Connection_Timeout_Dev"]
                ...
                q1 = """update control_stream set status = 1 WHERE program_name = 'Streaming_Dev_2.py' """
                q2 = """update control_stream set status = 2 WHERE program_name = 'Streaming_Dev_1.py' """
                ...
                cursor.close()
                connection.close()
                subprocess.Popen("python3 /dbfs/FileStore/tables/Streaming_Process/Dev/Streaming_Dev_2.py", shell=True)
            ...
            #Writing 429 error code information into the file at '/dbfs/FileStore/tables/Streaming_Process/Dev/Error_Log_Files/'
            Filename = '/dbfs/FileStore/tables/Streaming_Process/Dev/Error_Log_Files/Error_on_'
            Date = str(date.today())
            timestamp = str(time.time())
            FilePath = Filename + Date +"_"+timestamp
  ...
  def on_exception(self, exception):
    ...
    # Writing exception information into the file at '/dbfs/FileStore/tables/Streaming_Process/Dev/Error_Log_Files/'
    Filename = '/dbfs/FileStore/tables/Streaming_Process/Dev/Error_Log_Files/Exception_on_'
    Date = str(date.today())
    timestamp = str(time.time())
    FilePath = Filename + Date + "_" + timestamp
    ErrorAt = datetime.now()
...
try:
  ...
  response = client.get_secret_value(SecretId="Secrets_Dev")
...
DATABASE = secret["Database_Dev"]
CONNECTION_TIMEOUT = secret["Db_Connection_Timeout_Dev"]
STREAM_NAME = secret["Kinesis_Stream_Name_Dev"]
REGION_NAME = secret["Aws_Region_Name_Dev"]
AWS_ACCESS_KEY_ID = secret["Aws_Access_Key_Id_Dev"]
AWS_SECRET_ACCESS_KEY = secret["Aws_Secret_Access_Key_Dev"]
TWITTER_CONSUMER_KEY = secret["Consumer_Key_Dev_1"]
TWITTER_CONSUMER_SECRET = secret["Consumer_Secret_Dev_1"]
TWITTER_ACCESS_TOKEN = secret["Access_Token_Dev_1"]
TWITTER_ACCESS_TOKEN_SECRET = secret["Access_Token_Secret_Dev_1"]
...
if __name__ == "__main__":
    ...
    try:
        ...
        with open("/dbfs/FileStore/tables/Streaming_Process/Dev/CurrentTracking.txt","w") as output:
